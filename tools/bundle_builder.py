...
ACCENTS = {
 ...
 25:(0,'#d81b60'), 26:(0,'#b3261e'), 27:(0,'#e8641b'), 28:(1,'#e91e8c'), 29:(1,'#e8b64c'),
}
...
def bb_css(dark, acc):
    ...
    panel2= 'rgba(255,255,255,.09)' if dark else 'rgba(0,0,0,.07)'
    line  = 'rgba(255,255,255,.14)' if dark else 'rgba(0,0,0,.10)'
    soft  = 'rgba(255,255,255,.55)' if dark else 'rgba(0,0,0,.55)'
    boxbg = ('linear-gradient(180deg,rgba(255,255,255,.09),rgba(255,255,255,.04))' if dark
             else 'linear-gradient(180deg,#ffffff,#fcfafa)')
    dropbg= ('linear-gradient(180deg,rgba(255,255,255,.03),rgba(255,255,255,.06))' if dark
             else 'linear-gradient(180deg,#fdfdfd,#f4f2f3)')
    shadow= ('0 24px 50px -22px rgba(0,0,0,.65), 0 4px 14px -6px rgba(0,0,0,.4)' if dark
             else '0 24px 50px -24px rgba(40,20,50,.28), 0 4px 14px -8px rgba(40,20,50,.14)')
    accsoft=rgba_(acc,'.13'); accring=rgba_(acc,'.4'); accglow=rgba_(acc,'.32'); accdim=rgba_(acc,'.22')
    E='cubic-bezier(.22,.8,.26,1)'
    return '''
  /* ===== chum box builder (soft-depth, borderless) ===== */
  ...
  .bb-box{position:relative;background:'''+boxbg+''';border-radius:26px;
    padding:1rem 1rem 1.2rem;box-shadow:'''+shadow+'''}
  .bb-lid{height:14px;border-radius:13px 13px 0 0;margin:-1rem -1rem 14px;
    background:linear-gradient(180deg,'''+rgba_(acc,'.95')+''','''+rgba_(acc,'.75')+''');
    box-shadow:0 3px 8px -3px '''+accglow+'''}
  ...
  .bb-drop{position:relative;height:340px;border-radius:18px;overflow:hidden;background:'''+dropbg+''';
    box-shadow:inset 0 3px 12px rgba(0,0,0,.07), inset 0 -10px 22px rgba(0,0,0,.05);
    transition:box-shadow .45s '''+E+'''}
  ...
  .bb-drop.bb-has{box-shadow:inset 0 3px 12px rgba(0,0,0,.07), inset 0 -10px 22px rgba(0,0,0,.05),
    inset 0 0 0 1.5px '''+accdim+''', 0 0 26px -8px '''+accglow+'''}
  .bb-swirl{position:absolute;right:12px;bottom:9px;font-size:.72rem;font-weight:700;color:var(--bb-soft);opacity:0;
    pointer-events:none;transition:opacity .4s;letter-spacing:.06em;text-transform:uppercase}
  .bb-drop.bb-has .bb-swirl{opacity:.55}
  .bb-reset{border:0;background:transparent;color:var(--bb-soft);font-size:1.15rem;font-weight:800;cursor:pointer;
    line-height:1;padding:.1rem .3rem;transition:color .25s,transform .45s '''+E+'''}
  ...
  .bb-box.bb-land{animation:bb-squash .45s '''+E+'''}
  @keyframes bb-squash{35%{transform:scale(1.022,.975)}70%{transform:scale(.99,1.01)}100%{transform:scale(1)}}
  .bb-tabs{margin-top:12px;font-weight:700;font-size:.92rem;color:var(--bb-soft);text-align:center}
  /* --- summary + cta --- */
  .bb-sum{margin-top:1.1rem;background:var(--bb-panel);border-radius:20px;padding:1.05rem 1.2rem}
  ...
  .bb-cta{width:100%;margin-top:.9rem;border:0;border-radius:999px;padding:1.05rem;font-weight:800;font-size:1.05rem;
    background:linear-gradient(180deg,'''+rgba_(acc,'1')+''','''+rgba_(acc,'.85')+''');color:#fff;cursor:pointer;
    box-shadow:0 10px 24px -10px '''+accglow+''', inset 0 1px 0 rgba(255,255,255,.25);
    transition:transform .3s '''+E+''',box-shadow .3s '''+E+''',opacity .2s}
  .bb-cta:hover{transform:translateY(-2px);box-shadow:0 16px 32px -10px '''+accglow+''', inset 0 1px 0 rgba(255,255,255,.25)}
  .bb-cta:active{transform:scale(.97)}
  .bb-cta[disabled]{opacity:.45;cursor:not-allowed;box-shadow:none;transform:none}
  /* --- pickers --- */
  .bb-row{display:flex;align-items:center;gap:.9rem;background:var(--bb-panel);
    border-radius:20px;padding:.9rem 1.05rem;margin-bottom:.7rem;
    transition:background .35s '''+E+''',box-shadow .35s '''+E+''',transform .35s '''+E+'''}
  .bb-row:hover{transform:translateY(-2px);box-shadow:0 12px 26px -16px rgba(0,0,0,.35)}
  .bb-row.bb-on{background:'''+accsoft+''';box-shadow:inset 0 0 0 1.5px '''+accring+''', 0 10px 24px -16px '''+accglow+'''}
  .bb-dot{flex:0 0 44px;filter:drop-shadow(0 4px 6px rgba(0,0,0,.18))}
  ...
  .bb-size{border:0;background:'''+panel2+''';color:inherit;border-radius:999px;
    padding:.38rem .66rem;font-weight:700;font-size:.8rem;cursor:pointer;white-space:nowrap;
    transition:background .3s '''+E+''',color .3s,box-shadow .3s '''+E+''',transform .2s '''+E+'''}
  .bb-size:hover{transform:translateY(-1px)}
  .bb-size.bb-sel{background:var(--bb-acc);color:#fff;box-shadow:0 6px 14px -6px '''+accglow+'''}
  .bb-add{border:0;border-radius:999px;width:2.4rem;height:2.4rem;font-size:1.2rem;font-weight:800;cursor:pointer;
    background:var(--bb-acc);color:#fff;flex:0 0 auto;line-height:1;
    box-shadow:0 6px 14px -6px '''+accglow+''';transition:transform .25s '''+E+''',box-shadow .25s '''+E+'''}
  .bb-add:hover{transform:translateY(-2px)}
  .bb-add:active{transform:scale(.9)}
  .bb-qtywrap{display:flex;align-items:center;gap:.35rem;flex:0 0 auto}
  .bb-minus{border:0;background:'''+accsoft+''';color:var(--bb-acc);border-radius:999px;width:2.4rem;height:2.4rem;
    font-size:1.2rem;font-weight:800;cursor:pointer;line-height:1;transition:transform .25s '''+E+''',background .3s}
  .bb-minus:hover{background:'''+accdim+'''}
...
(function(){
  ...
  var sprites={}, spriteR=0;
  function shade(hex,amt){
    var n=parseInt(hex.slice(1),16),r=n>>16,g=(n>>8)&255,b=n&255,t=amt>0?255:0,a=Math.abs(amt);
    return 'rgb('+Math.round(r+(t-r)*a)+','+Math.round(g+(t-g)*a)+','+Math.round(b+(t-b)*a)+')';
  }
  function sprite(c){ // pre-baked realistic gummy: radial body, gloss, rim
    if(Math.abs(spriteR-R)>.4){sprites={};spriteR=R;}
    var sp=sprites[c]; if(sp)return sp;
    var d=Math.ceil(R*2+6), off=document.createElement('canvas');
    off.width=off.height=Math.ceil(d*DPR);
    var o=off.getContext('2d'); o.scale(DPR,DPR); var m=d/2;
    var g=o.createRadialGradient(m-R*.38,m-R*.42,R*.12,m,m,R*1.06);
    g.addColorStop(0,shade(c,.55)); g.addColorStop(.42,c); g.addColorStop(1,shade(c,-.24));
    o.fillStyle=g; o.beginPath(); o.arc(m,m,R,0,6.2832); o.fill();
    o.globalAlpha=.65; o.fillStyle='#fff'; o.beginPath();
    o.ellipse(m-R*.34,m-R*.44,R*.34,R*.18,-.55,0,6.2832); o.fill();
    o.globalAlpha=.22; o.beginPath();
    o.ellipse(m+R*.3,m+R*.44,R*.2,R*.1,.5,0,6.2832); o.fill();
    o.globalAlpha=1;
    sprites[c]=off; return off;
  }
  function draw(){
    cx.clearRect(0,0,W,H);
    // glass interior: edge vignette + floor gradient + shine streaks
    var eg=cx.createLinearGradient(0,0,W,0);
    eg.addColorStop(0,'rgba(0,0,0,.05)');eg.addColorStop(.09,'rgba(0,0,0,0)');
    eg.addColorStop(.91,'rgba(0,0,0,0)');eg.addColorStop(1,'rgba(0,0,0,.05)');
    cx.fillStyle=eg;cx.fillRect(0,0,W,H);
    var fg=cx.createLinearGradient(0,H-36,0,H);
    fg.addColorStop(0,'rgba(0,0,0,0)');fg.addColorStop(1,'rgba(0,0,0,.09)');
    cx.fillStyle=fg;cx.fillRect(0,H-36,W,36);
    ...
    cx.restore();
    // soft contact shadows under every gummy
    cx.fillStyle='rgba(0,0,0,.07)';
    for(var i=0;i<bodies.length;i++){var b=bodies[i];
      cx.beginPath();cx.ellipse(b.x,Math.min(b.y+R*.8,H-2.5),R*.82,R*.26,0,0,6.2832);cx.fill();}
    // gummies via cached sprites (fast + rich shading)
    for(i=0;i<bodies.length;i++){b=bodies[i];
      var sp=sprite(b.c), d=sp.width/DPR;
      cx.save();cx.translate(b.x,b.y);cx.rotate(b.rot);cx.scale(b.sq,1/b.sq);
      cx.drawImage(sp,-d/2,-d/2,d,d);cx.restore();
    }
  }
